...
if ($('.err').size()==0 && blockscount==0)
{
    ...
    $('#timer').stopTime();
    window.game_record = parseInt(100000/window.game_time);

    /*ajax begins*/
    var record = JSON.stringify ({ name:$('#name').val() , record:game_record });
    $.ajax({
        type : 'post' ,
        contentType : 'application/json' ,
        url : 'http://rabbitpiyoko.com/test_1/uploadRecordPost/' ,
        processDate : false ,
        dataType : json ,
        data : record ,
        success : function(){},
...
if(l!=0)$('#timer').everyTime(1000,'timer',function(i){
    $(this).html('Timer:<span>'+f(Math.floor(i/3600))+':'+f(Math.floor(i/60))+':'+f(i%60)+'</span>').find('span').css({'font-weight': '100'});})
    window.game_time = i;
}
...
jQuery.fn.extend({
    ...
    if(true) {finit(l);}
    $('td[id*=c_].c').click(function(){
      $this = $(this);
      var input = prompt('请输入0~9的数值');
      $this.setnum(parseInt(input));
      $this.check();
    });
},
    congtl: function() {
